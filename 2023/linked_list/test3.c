...
void BookList_outputDataWithPrice(Node *node, size_t amount, float price)
{
    printf("%ld\n", amount);
    Node *cur = node;
    while (cur) {
        if (cur->book.price == price)
...
int main()
{
    ...
    size_t amount = 0;
    float max_price = BookList_getMaxPrice(list.next, &amount);
    BookList_outputDataWithPrice(list.next, amount, max_price);
